...
| Command | Description |
|---------|-------------|
| `{{CMD_REF:parallel}}` | Start Multi-Agent Pipeline |
| `{{CMD_REF:start}}` | Start normal development mode (single process) |
| `{{CMD_REF:finish-work}}` | Pre-completion checklist + record session |
